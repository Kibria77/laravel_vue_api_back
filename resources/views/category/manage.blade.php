                ...
                <div class="card-body">
                    ...
                    <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100">
                        ...
                        <tr>
                            ...
                            <td>
                                <img src="{{ asset($category->image) }}" alt="" class="img-thumbnail" height="80" width="80"/>
                            </td>
                            <td>
                                <button class="{{$category->status == 1 ? 'btn btn-success btn-sm' : 'btn btn-danger btn-sm'}}">{{$category->status == 1 ? 'Publish' : 'Unpublished'}}</button>
                            </td>
                            <td class="text-center">
                                <a href="{{ route('update-category-status', ['id' => $category->id]) }}" class="btn {{ $category->status == 1 ? 'btn-info' : 'btn-warning' }} btn-sm">
                                    <i class="{{ $category->status == 1 ? 'fas fa-arrow-alt-circle-up' : 'fas fa-arrow-alt-circle-down' }}"></i>
                                </a>
                                <a href="{{ route('edit-category-info', ['id' => $category->id]) }}" class="btn btn-primary btn-sm">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="" class="btn btn-danger btn-sm" onclick="event.preventDefault(); document.getElementById('deleteCategory{{$category->id}}').submit();">
                                    <i class="fas fa-trash-alt"></i>
                                </a>
                                <form action="{{ route('delete-category', ['id' => $category->id]) }}" method="POST" id="deleteCategory{{$category->id}}">
                                    @csrf
                                </form>
                            </td>
                        </tr>
